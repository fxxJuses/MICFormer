...
class MDiceLoss(nn.Module):
    ...
    def __init__(self, do_sigmoid=True):
        ...
        self.do_sigmoid = do_sigmoid
        self.labels = ['backgroud','CT-A', 'CT-B', 'CT-C', 'CT-D', 'CT-E', 'CT-F', 'CT-G']
        # self.labels = ['CT-A']
        self.device = "cpu"
...
class MDiceLoss_Val(nn.Module):
    """Dice loss tailored to Brats need.
    """

    def __init__(self, do_sigmoid=True):
        super(MDiceLoss_Val, self).__init__()
        self.do_sigmoid = do_sigmoid
        self.labels = ['backgroud','CT-A', 'CT-B', 'CT-C', 'CT-D', 'CT-E', 'CT-F', 'CT-G']
        self.device = "cpu"
        ...
        if metric_mode:
            ...
            if targets.sum() == 0:
                ...
                if inputs.sum() == 0:
                    ...
                    return torch.tensor(0., device="cuda")
            # Threshold the pred
        intersection = MDiceLoss_Val.compute_intersection(inputs, targets)
        if metric_mode:
            dice = (2 * intersection) / ((inputs.sum() + targets.sum()) * 1.0)
        else:
            dice = (2 * intersection + smooth) / (inputs.pow(2).sum() + targets.pow(2).sum() + smooth)
