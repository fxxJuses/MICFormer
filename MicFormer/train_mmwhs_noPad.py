...
from MMWHS_pre.Multi_modal.SymCFNet.models.MICFormer_self import Head 
...
def main(args):
    ...
    model_1 = Head(embed_dim=48 ,num_classes=args.num_classes)

    # model_1.load_from(config)

    if args.resume:
        args.checkpoint = args.checkpoint_folder / "model_best.pth.tar"
        args.start_epoch = reload_ckpt_bis(args.checkpoint, model_1,device="cuda:2")
        ...
        try:
            ...
            if (epoch + 1) % args.val == 0:
                ...
                if validation_loss_1 < best_loss:
                    print(f"Saving the model with loss {validation_loss_1}")
