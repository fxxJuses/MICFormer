...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        """Forward function."""
        # x -> 1 1 128 128 128
        x = self.patch_embed(x) #  x - > 1 192 32 32 32
        down=[]
       
        Ws, Wh, Ww = x.size(2), x.size(3), x.size(4)
        
        x = x.flatten(2).transpose(1, 2).contiguous() # 1 32768 192
        x = self.pos_drop(x) # x shape : 1 32768 192 
...
if __name__ == "__main__":
    ...
    model = nnFormer(crop_size=[128,128,128],
                ...
                num_heads=[6, 12, 24, 48],
                patch_size=[4,4,4],
                window_size=[4,4,8,4]).cuda()
    inputs =  torch.randn(size=(1,1,128,128,128)).cuda()
    output = model(inputs)
